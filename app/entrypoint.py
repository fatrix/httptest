def func(self):
    ...
    if id:
        ...
        if "FRONTEND_API_URL" in self.settings:
            return self.responses.RedirectResponse("https://%s/api/?testid=%s&version=%s" % (self.settings.FRONTEND_BASE_URL, new_id, version))
        else:
    ...
    if self.method == "GET" and self.GET.has_key("sendmail"):
        ...
        if "FRONTEND_API_URL" in self.settings:
            return self.responses.RedirectResponse("https://%s/fastapp/httptest/static/index.html?testid=%s&version=%s" % (self.settings.FRONTEND_BASE_URL, id, version))
        else:
